...
document.querySelector('#storeForm').onsubmit=async e=>{e.preventDefault();if(typeof catalogBusy!=='undefined'&&catalogBusy)return;const next=Object.fromEntries(new FormData(e.target));for(const key of Object.keys(next))next[key]=next[key].trim();const status=document.querySelector('#storeStatus');if(['title','bank','account','holder','copyLabel','submitLabel'].some(key=>!next[key])){status.textContent='제목, 계좌 정보와 버튼 문구를 입력해 주세요.';return;}if(!/^[0-9\s-]+$/.test(next.account)){status.textContent='계좌번호는 숫자, 공백, 하이픈으로 입력해 주세요.';return;}try{sellerStorage.setItem('live-shop-store-settings-v1',JSON.stringify(next));storeSettings=next;applyStoreSettings();status.textContent='고객 화면에 반영 중…';if(typeof runCatalog==='function'){const ok=await runCatalog('save',{fromProduct:true});status.textContent=ok?'설정 저장 및 고객 화면 반영 완료':'이 브라우저에 보관되었습니다. 반영 미완료: '+document.querySelector('#catalogSyncStatus').textContent+' 설정 저장을 다시 눌러 주세요.';}else status.textContent='이 브라우저에만 저장되었습니다.';}catch{status.textContent='설정을 저장하지 못했습니다. 브라우저 저장 공간과 설정을 확인해 주세요.';}};
let copyTimer;
