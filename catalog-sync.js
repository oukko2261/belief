...
async function cancelCustomerOrder(order,button){
  if(!confirm(order.buyer.name+'님의 주문을 취소할까요? 주문번호: '+order.id+'\n입금 완료 주문이라면 재고가 복구됩니다. 실제 환불은 자동으로 진행되지 않습니다.'))return;
  const status=document.querySelector('#orderSheetStatus');button.disabled=true;status.textContent='주문 취소 처리 중…';
  try{const login=await (await fetch('/api/google/status')).json();if(!login.authenticated)throw Error('Google 로그인 후 다시 시도해 주세요.');const response=await fetch('/api/orders/cancel',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':login.csrf},body:JSON.stringify({orderId:order.id})});const result=await response.json();if(!response.ok)throw Error(result.error||'취소 실패');status.textContent='주문 취소 완료. '+(result.sheetError?'시트 반영 실패: '+result.sheetError+' ‘주문을 구글시트에 기록’으로 다시 시도해 주세요.':'구글시트에도 취소 상태를 기록했습니다.');await document.querySelector('#refreshCustomerOrders').onclick();}catch(error){status.textContent=error.message+' 주문 목록을 새로고침해 상태를 확인하세요.';button.disabled=false;}
}
async function markCustomerOrderPaid(order,button){
  if(!confirm(order.buyer.name+'님의 입금을 확인했나요?\n입금 완료로 바꾸면 재고가 차감되고 발주 대상이 됩니다.'))return;
  const status=document.querySelector('#orderSheetStatus');button.disabled=true;status.textContent='입금 완료 처리 중…';
  try{const login=await (await fetch('/api/google/status')).json();if(!login.authenticated)throw Error('Google 로그인 후 다시 시도해 주세요.');const response=await fetch('/api/orders/paid',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':login.csrf},body:JSON.stringify({orderId:order.id})});const result=await response.json();if(!response.ok)throw Error(result.error||'입금 완료 처리 실패');status.textContent='입금 완료 · 재고 차감 및 발주 대상 반영. '+(result.sheetError?'시트 반영 실패: '+result.sheetError+' ‘주문을 구글시트에 기록’으로 다시 시도해 주세요.':'구글시트에도 입금 완료를 기록했습니다.');await document.querySelector('#refreshCustomerOrders').onclick();}catch(error){status.textContent=error.message+' 주문 목록을 새로고침해 상태를 확인하세요.';button.disabled=false;}
}
...
document.querySelector('#refreshCustomerOrders').onclick=async()=>{
  ...
  try{const login=await (await fetch('/api/google/status')).json();if(!login.authenticated)throw Error('Google 로그인 후 확인해 주세요.');status.textContent='주문 확인 중…';const response=await fetch('/api/orders',{method:'POST',headers:{'X-CSRF-Token':login.csrf}});const result=await response.json();if(!response.ok)throw Error(result.error||'주문 확인 실패');for(const order of result.orders.slice().reverse()){const card=document.createElement('section');card.className='sale-action-group';const heading=document.createElement('h4');heading.textContent=order.buyer.name+' · '+order.total.toLocaleString('ko-KR')+'원';const content=document.createElement('p');content.style.whiteSpace='pre-wrap';const label=order.status==='paid'?'입금 완료 · 재고 차감됨':order.status==='cancelled'?'취소':'입금 대기 · 재고 미차감';content.textContent=label+' · '+order.id+'\n'+order.date+' / '+order.buyer.phone+'\n입금자명: '+(order.buyer.depositor||order.buyer.name)+'\n'+order.buyer.address+'\n'+order.items.map(i=>i.name+' '+Object.values(i.options).join('/')+' × '+i.quantity).join('\n');card.append(heading,content);if(order.status!=='paid'&&order.status!=='cancelled'){const paid=document.createElement('button');paid.type='button';paid.className='submit';paid.textContent='입금 완료';paid.onclick=()=>markCustomerOrderPaid(order,paid);card.append(paid);}const cancel=document.createElement('button');cancel.type='button';cancel.className='copy';cancel.textContent=order.status==='cancelled'?'취소 완료':'주문 취소';cancel.disabled=order.status==='cancelled';cancel.onclick=()=>cancelCustomerOrder(order,cancel);card.append(cancel);list.append(card);}status.textContent=result.orders.length+'건 · 구글시트에 자동 기록됩니다. 누락된 경우에만 ‘시트 기록 재시도’를 이용하세요.';}catch(error){status.textContent=error.message;}finally{button.disabled=false;}
};
