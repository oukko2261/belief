...
window.add=id=>{
  ...
  if(p.allowCustomOption){const wrapper=document.createElement('label');wrapper.textContent=p.customOptionLabel||'옵션';const input=document.createElement('select');input.name='custom';input.required=true;input.add(new Option('선택해 주세요',''));for(const value of p.customOptions||[])input.add(new Option(value,value));wrapper.append(input);form.append(wrapper);if(!p.customOptions?.length){const note=document.createElement('p');note.textContent='옵션 목록 준비 중입니다. 판매자에게 문의해 주세요.';form.append(note);}}
