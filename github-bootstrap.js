...
const customerQuickView=document.querySelector('#customerQuickView');const updateCustomerQuickView=()=>{if(!customerQuickView)return;const customerUrl=new URL('https://live-order-mobile-test.wooks88.chatgpt.site/shop/'+login.sellerId);const date=document.querySelector('#saleDate')?.value;if(/^\d{4}-\d{2}-\d{2}$/.test(date||''))customerUrl.searchParams.set('date',date);if(typeof activeSaleKey!=='undefined'&&activeSaleKey!==date)customerUrl.searchParams.set('page',activeSaleKey);customerQuickView.href=customerUrl.href;customerQuickView.textContent=date?'고객 화면 보기 · '+date:'고객 화면 보기';customerQuickView.hidden=false;};updateCustomerQuickView();
   const keys={products:'live-shop-products-v1',salePages:'live-shop-sale-pages-v1',salePageNames:'live-shop-sale-names-v1',salePageStatus:'live-shop-sale-status-v1',storeSettings:'live-shop-store-settings-v1'};
   if(sellerStorage.getItem(keys.products)===null){let data={products:[],salePages:{},salePageNames:{},salePageStatus:{},storeSettings:{}};if(login.sheetUrl){const r=await fetch('/api/catalog/load',{method:'POST',headers:{'X-CSRF-Token':login.csrf}});const saved=await r.json();if(!r.ok)throw Error(saved.error||'시트에서 불러오지 못했습니다.');if(!saved.empty)data=saved.data;}for(const [key,path]of Object.entries(keys))sellerStorage.setItem(path,JSON.stringify(data[key]||{}));}
   for(const src of ['app.js','google-connect.js','store-settings.js','sale-pages.js','sale-pdf.js','inline-copy.js','live-order.js','product-options.js','catalog-sync.js'])await new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='./'+src+'?v=single-customer-link-1';script.onload=resolve;script.onerror=reject;document.body.append(script);});
   document.querySelector('#adminToggle').click();
   document.querySelector('#saleDate')?.addEventListener('change',updateCustomerQuickView);document.querySelector('#loadSalePage')?.addEventListener('click',()=>setTimeout(updateCustomerQuickView));document.querySelector('#saveCatalog')?.addEventListener('click',()=>setTimeout(updateCustomerQuickView));;updateCustomerQuickView()
